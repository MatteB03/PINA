# alternative version where the sample isn't focused only on 
# the border but rather in two circular crowns near it as well
...
# create data like it's done in the thesis 
...
# and in two circular crowns near it inside and outside the circle
circle_border = EllipsoidDomain({"x":[25,75], "y":[25,75], "t":[0,0]}, sample_surface=True)
circle = EllipsoidDomain({"x":[25,75], "y":[25,75], "t":[0,0]})
circle1 = EllipsoidDomain({"x":[25.5,74.5], "y":[25.5,74.5], "t":[0,0]})
circle2 = EllipsoidDomain({"x":[25.1,74.9], "y":[25.1,74.9], "t":[0,0]})
circle3 = EllipsoidDomain({"x":[24.9,75.1], "y":[24.9,75.1], "t":[0,0]})
circle4 = EllipsoidDomain({"x":[24.5,75.5], "y":[24.5,75.5], "t":[0,0]})
outside_circle = Difference([CartesianDomain({"x": [0, 100],"y": [0, 100], "t": [0,0]}),circle])
inner_crown = Difference([circle2, circle1])# just inside, value will be 1
outer_crown = Difference([circle4, circle3])# just outside, value will be 0

# create the problem
class MonodomainProblem(TimeDependentProblem, SpatialProblem):
    
    output_variables = ["u"]
    spatial_domain = CartesianDomain({"x": [0, 100],"y": [0, 100]})
    temporal_domain = CartesianDomain({"t": [0, 2]})
    ...
    conditions = {
        ...
        "inner_crown": Condition(
            domain= inner_crown,
            equation= FixedValue(1)),
        "outer_crown": Condition(
            domain= outer_crown,
            equation= FixedValue(0))
        }
...
problem.discretise_domain(5000, "random", domains=["u_1_border"])
problem.discretise_domain(5000, "random", domains=["inner_crown"])
problem.discretise_domain(5000, "random", domains=["outer_crown"])
...
for idx, t_val in enumerate(time_points):
    ...
    input_test = CartesianDomain({"x": [0, 100], "y": [0, 100], "t": t_val}).sample(200, mode="grid")
