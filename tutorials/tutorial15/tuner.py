...
'''def train_pinn(config):
    ...
    problem.discretise_domain(1000)
    ...
    trainer = Trainer(solver, max_epochs=1000, accelerator='cpu', enable_progress_bar=False, log_every_n_steps=0, callbacks=[TuneReportCallback(metrics={"loss": "val_loss"}, on="train_end")])
    trainer.train()
config = {
    "n_layers": tune.choice([2, 3]),
    "inner_size": tune.choice([10, 20]),
    "func": tune.choice([torch.nn.Tanh]),
    "solver" : tune.choice([PINN, RBAPINN]),
}
# 4. Set up the Ray Tune run
...
# 5. Get the best hyperparameters found by Ray Tune
